...
public class EventCoordinator {
    ...
    public void publish(Event event){
        log.info("XfylEventCoordinator->publish event={}", event);
        try {
            // 构建同步事件消费者处理日志
            ...
            log.error("XfylEventCoordinator->publish error", e);
    ...
    public void disposeAsyncEvent(Long eventId, String consumerCode) {
        log.info("XfylEventCoordinator->disposeAsyncEvent eventId={}，consumerCode={}", eventId, consumerCode);
        ...
        if (!wrapper.getAsync()){
            log.info("XfylEventCoordinator->disposeAsyncEvent wrapper is sync break");
            return;
        }
